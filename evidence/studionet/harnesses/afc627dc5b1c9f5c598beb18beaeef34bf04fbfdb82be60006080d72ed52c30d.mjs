...
export async function runLive(env = process.env) {
  ...
  async function transact(method, args, actor = owner) {
    const event = {
      method,
      inputs: args,
    ...
    for (let attempt = 0; attempt < 120; attempt += 1) {
      ...
      if (status === 'FINALIZED') {
        ...
        assert.equal(
          state.state,
          'success',
          method + ': execution success not proven by app parser',
        );
        if (method === 'deploy') {
  ...
  try {
    ...
    for (const [nonce, draft, verdict, reasons] of cases) {
      const expected = {
        name: nonce,
        question,
        draft,
    ...
    const good = evidence.cases[0].output;
    await transact(
      'publish_answer_card',
    ...
    evidence.limitations = [
      ...
      'Three basic model cases, not the full adversarial matrix.',
